...
import { fetchCreditHistory, fetchDownloadCreditHistory, fetchWorkspace } from '../../../utils/ApiClient';
...
export default function CreditHistory() {
    ...
    const fetchData = async (type, page, searchkey) => {
        ...
        // Include customer_id only if it's not null
        if (customer_id) {
            api_input.customer_id = customer_id;
        }
        ...
        try {
            const response = await fetchCreditHistory(api_input);
    ...
    const downloadCsv = async () => {
        try {
            ...
            const response = await fetchDownloadCreditHistory(api_input);
            const response_data = response.data.results;
            const csv_data = response_data.data;
            exportToCsv(csv_data, "credit_history");
        } catch (error) {
        ...
        <>
            <div id="content-page" className="content-page">
                <div className="container">
                    ...
                    <PageTitle heading="Credit History" />
                                                        ...
                                                        {data?.length > 0 ? (
                                                            data.map((item, index) => (
                                                                <tr key={index}>
                                                                    ...
                                                                    <td>
                                                                        <span className={`font-size-15 ${item.payment_type === "credit" ? "text-success" : "text-danger"}`}>
                                                                            {item.amount_collected}
                                                                        </span>
                                                                    </td>
                                                                    <td>
