...
const PlayListVideoModal = ({ video, onHide, onVideoDeleted, fetchVideos }) => {
  ...
  //       fetchVideos();
  //     } else {
  //       console.error("Error editing video:", response.data.message);
  //       triggerAlert(
  //         "error",
  //         "Oops...",
  //         `Error editing video: ${response.data.message} (Status: ${response.status})`
  //       );
  //       setShouldCloseModal(true);
  //     }
  //   } catch (error) {
  ...
  //   if (!video.post_id) {
  //     triggerAlert("error", "Oops...", "Please select a video");
  //     return;
  //   }
  ...
  //   setTimeout(() => {
  //     ConfirmationAlert('Are you sure you want to delete this video?', 'Continue', async () => {
  //       setIsLoading(true);
  ...
  //         if (response.status === 200) {
  //           triggerAlert("success", "success", "Video deleted Successfully!!");
  ...
  const handleEditVideo = async (data) => {
    try {
      ...
      if (response.status === 200) {
        ...
        // Refresh the video list with the updated data
        await fetchVideos(video.post_type || 'video', true);
      ...
      ConfirmationAlert('Are you sure you want to delete this video?', 'Continue', async () => {
        ...
        try {
          ...
          if (response.status === 200) {
            ...
            // Refresh the video list with reset=true to start fresh
            await fetchVideos(video.post_type || 'video', true);
          } else {
            console.error("Error deleting video:", response.data.message);
            triggerAlert(
